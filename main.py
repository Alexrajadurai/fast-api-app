...
def model_predict(images_arr): 
    ...
    preds_probablity = np.array(predictions)
    argmaxs = np.argmax(preds_probablity, axis = 1)
...
async def upload_files(files: List[UploadFile] = File(...)):
    ...
    resized_img = [resize(img) for img in images]
    images_rgb = [cv2.cvtColor(img, cv2.COLOR_BGR2RGB ) for img in resized_img ]
    ...
    columns_labels = ["Image", "Predictions"]

    table_html = html_table(image_paths, class_predictions, columns_labels)

    content = head_html + """
    <marquee width="525" behaviour="alternate"<h1 style="color:red;font-family:Arial">Here's Our Prediction! </h1></marquee>
...
async def main():
    content = head_html + """
            <marquee width="525" behaviour="alternate"><h1 style="color:red";font-family:Arial> Upload Pictures!! </h1></marquee>
            <h3 style="font-family:Arial"> We'll Try to Predict Which Categories They Are:</h3><br>
            """
    ...
    content = content + html_table(full_original_paths,display_names,column_labels)
...
def html_table(image_paths,names,column_labels):
